...
const createRoutes = (routes, basePath='/') => {
    const finalRoutes = routes.map((route) => {
        ...
        return {
            ...route,
            path: route.path + '/*',
            element: <Suspense fallback={<GenericPage details="Loading..." />}>
                <route.element label={route.label} mountPath={mountPath} data={route?.data} links={links} />
            </Suspense>,
            errorElement: <GenericPage hasImage={false} message={<i className="fa-solid fa-circle-exclamation px-2 text-danger" />} details="Something went wrong"/>,
            children: children
        }
    ...
    finalRoutes.push({
        index: 'true',
        element: <GenericPage hasImage={false} message={<i className="fa-solid fa-gears p-2" />} details="Choose an integration"/>
        // element:
    })
